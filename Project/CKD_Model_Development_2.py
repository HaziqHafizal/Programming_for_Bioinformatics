...
# Focus: Extra Trees Classifier
# ---------------------------------------------------------
print("\n=== PART A: MODEL REFINEMENT (Extra Trees Classifier) ===")
...
if 'classification' in df.columns:
    ...
    grid_search = GridSearchCV(estimator=etc, param_grid=param_grid, cv=5, n_jobs=-1, verbose=1)
    ...
    cv_scores = cross_val_score(best_etc, X, y, cv=10)
    print(f"\n--- Overfitting Analysis ---")
    print(f"10-Fold Cross-Validation Mean Score: {cv_scores.mean():.4f}")
    print(f"Training Score (on full train set): {best_etc.score(X_train, y_train):.4f}")

    if best_etc.score(X_train, y_train) - cv_scores.mean() > 0.1:
